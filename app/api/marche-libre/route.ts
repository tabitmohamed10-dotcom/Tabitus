...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let q = supabase
      ...
      .select(`
        id, title, price, city, condition, status,
        category, images, is_negotiable, views, created_at,
        seller:user_id (full_name, avatar_url, role)
      `, { count: 'exact' })
      .eq('status', 'active')
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title, description, price, category_slug, city, condition, images, contact_method, price_negotiable } = body
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        is_negotiable: price_negotiable ?? true,
        status: 'active',
      })
      .select()
      .maybeSingle()
